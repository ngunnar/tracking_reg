...
def get_nconv_flow_multi(points_t, flow_points, scores, shape, model):
    ...
    for idx in range(len(points_t)):
        if points_t[idx][0] >= shape[0] or points_t[idx][1] >= shape[1] or points_t[idx][0] < 0 or points_t[idx][1] < 0:
            continue
        c[points_t[idx][1], points_t[idx][0]] = 1.0 #scores[idx]
        x1[points_t[idx][1], points_t[idx][0]] = flow_points[idx][0]
        x2[points_t[idx][1], points_t[idx][0]] = flow_points[idx][1]
...
def get_tps_flow_multi(points_t, flow_points, shape):    
    b = []
    ...
    flow = np.stack([z_yi, z_xi], axis=-1)
    return flow
