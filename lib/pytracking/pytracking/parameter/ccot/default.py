...
from pytracking.features import deep, color
import torch
...
def parameters(gpu_device = 0):
    ...
    shallow_params.translation_weight = 0.1                      # Weight of this feature
    deep_params.translation_weight = 1 - shallow_params.translation_weight

    # Init augmentation parameters
    params.augmentation = {'fliplr': False,#True,
    ...
    features_hog = deep.HOG(use_gpu=params.use_gpu, 
                               gpu_device = gpu_device, 
                               fparams=FeatureParams(feature_params=[shallow_params]), 
                               pool_stride=[2, 1], 
                               normalize_power=2)
    
    params.features = MultiResolutionExtractor([features, features_hog])
